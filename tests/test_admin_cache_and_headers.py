...
def test_lead_list_cache_is_invalidated_after_new_chat() -> None:
    ...
    before = client.get("/admin/leads?limit=9999", headers=ADMIN)
    ...
    after = client.get("/admin/leads?limit=9999", headers=ADMIN)
...
def test_appointment_list_cache_is_invalidated_after_confirmation() -> None:
    ...
    before = client.get("/admin/appointments?limit=9999", headers=ADMIN)
    assert before.status_code == 200
    before_count = len(before.json())
    ...
    after = client.get("/admin/appointments?limit=9999", headers=ADMIN)
    assert after.status_code == 200
    after_items = after.json()
    assert len(after_items) >= before_count + 1
